...
import {
    CssBaseline, Box, Container, Paper, Typography, Button,
    TextField, Checkbox, FormControlLabel, Grid
} from '@mui/material';

function EditarRegistro() {
    const navigate = useNavigate();
    const { id } = useParams(); // Pega o ID do registro da URL (ex: /editar-registro/5)
    ...
    // useEffect para buscar os dados do registro específico quando a página carrega
    useEffect(() => {
        ...
        const fetchRegistro = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    // Preenche o formulário com os dados que vieram do backend
                    setFormData({
                        peso: data.peso || '',
        ...
        fetchRegistro();
    }, [id, navigate]); // Roda o efeito se o ID ou a função navigate mudarem
