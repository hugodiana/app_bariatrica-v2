...
function EditarRegistro({ handleNotification }) {
    ...
    useEffect(() => {
        ...
        const fetchRegistro = async () => {
            try {
                // CORREÇÃO AQUI
                const response = await fetch(`${process.env.REACT_APP_API_URL}/api/acompanhamento/registros/${id}/`, { headers: { 'Authorization': `Token ${token}` } });
                if (response.ok) {
                    const data = await response.json();
                    setFormData({ peso: data.peso || '', agua_ml: data.agua_ml || '', vitaminas_tomadas: data.vitaminas_tomadas || false, observacoes: data.observacoes || '' });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // CORREÇÃO AQUI
            const response = await fetch(`${process.env.REACT_APP_API_URL}/api/acompanhamento/registros/${id}/`, {
                method: 'PUT',
    ...
    return (
        <Container component="main" maxWidth="md"><CssBaseline /><Paper sx={{ p: 3, mt: 5 }}><Typography component="h1" variant="h5" gutterBottom>Editar Registro</Typography><Box component="form" onSubmit={handleSubmit} noValidate sx={{ mt: 1 }}><Grid container spacing={2}><Grid item xs={12} sm={6}><TextField type="number" name="peso" label="Peso (kg)" value={formData.peso} onChange={handleChange} fullWidth /></Grid><Grid item xs={12} sm={6}><TextField type="number" name="agua_ml" label="Água (ml)" value={formData.agua_ml} onChange={handleChange} fullWidth /></Grid><Grid item xs={12}><FormControlLabel control={<Checkbox checked={formData.vitaminas_tomadas} onChange={handleChange} name="vitaminas_tomadas" />} label="Tomei as vitaminas" /></Grid><Grid item xs={12}><TextField name="observacoes" label="Observações do dia..." value={formData.observacoes} onChange={handleChange} multiline rows={4} fullWidth /></Grid></Grid><Box sx={{ mt: 3, display: 'flex', justifyContent: 'space-between' }}><Button variant="outlined" onClick={() => navigate('/painel')}>Cancelar</Button><Button type="submit" variant="contained">Salvar Alterações</Button></Box></Box></Paper></Container>
    );
}
