...
function RequestPasswordReset({ handleNotification }) {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            // A CORREÇÃO ESTÁ AQUI: Removemos "const response ="
            await fetch(`${process.env.REACT_APP_API_URL}/api/auth/password/reset/`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ email: email }),
            });
            
            // Independentemente da resposta, mostramos uma mensagem genérica por segurança.
